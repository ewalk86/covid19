...
age_count_month_plot2 <- covid_data %>% 
   ...
   ggplot() +
      ...
      geom_label(aes(x = Sys.Date()-9, y = 75, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
      theme_minimal() +
      labs(title = "Weekly COVID-19 cases by age group",
...
contacts_age_plot <- covid_data %>% 
   ...
   ggplot() +
      geom_rect(aes(xmin = Sys.Date() - 14, xmax = Sys.Date(), ymin=0, ymax=Inf),
               fill = "gray80", alpha = 0.05) +
      geom_label(aes(x = Sys.Date()-9, y = 15, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
...
sewage_data <- 
   read_xlsx(paste0(input_path, "Msla WW COVID graph.xlsx"), sheet = 1,
             range = "A2:G26", 
                        col_names = c("date", "n1_gene", "n2_gene", 
                                      "daily_inc", "new_case_7day_ave", 
                                      "active_cases", "active_cases_7day_ave"),
                        col_types = c("date", "numeric", "numeric",
                                      "numeric", "numeric",
                                      "numeric", "numeric"))

sewage_model <- lm(new_case_7day_ave ~ n1_gene, data = sewage_data)
