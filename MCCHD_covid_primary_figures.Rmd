...
covid_incidence <- county_state_joined %>% 
   ggplot() +
     ...
     theme(strip.text = element_text(size = 16, colour = "black"),
           ...
           axis.text.x = element_text(size = 12, colour = "black", vjust = 0.4),
...
kable(covid_summary, caption = "Missoula County COVID-19 Cases",
   ...
   pack_rows("Travel History", 6, 8) %>% 
   pack_rows("Contact with Known Case", 9, 11) %>% 
...
covid_plot_r <- county_r_clean %>% 
   ggplot() +
     ...
     geom_label(aes(x = Sys.Date()-6, y = 2.5, 
     ...
     ylab("Rolling 7-day mean R-value") +
     xlab(" ") +
