...
county_dates_new <- county_dates_new[-(1:7), 1:2]
...
covid_plot_onset <- covid_data %>% 
   ggplot() +
     ...
     geom_line(data = county_r_clean, aes(dates, mean_r, color = label), size = 1.5) +
...
covid_plot <- covid_data %>% 
   ...
   ggplot(aes(x = variable, y = values, fill = gender)) +   
     ...
     scale_y_continuous(breaks = c(seq(0, 50, 2)), labels = c(seq(0, 50, 2))) +
     coord_flip() +
     theme_minimal() +
