...
date: "Data as of October 12, 2020"
output: pdf_document
header-includes:
...
age_count_plot <- covid_data %>% 
   ...
   ggplot() +   
     ...
     scale_y_continuous(breaks = c(seq(0, 1000, 25)), labels = c(seq(0, 1000, 25))) +
     coord_flip() +
     theme_minimal() +
...
age_incidence_plot_bar <- age_incidence %>% 
   ...
   ggplot() +
     ...
     scale_y_continuous(breaks = c(seq(0, 3000, 100)), labels = c(seq(0, 3000, 100))) +
     theme_minimal() +
     theme(strip.text = element_text(size = 16, colour = "black"),
           title = element_text(size = 18, colour = "black"),
           panel.grid = element_blank(),
...
epi_curve_onset_recent <- plot_data %>% 
   ...
   ggplot() +
     ...
     geom_text(aes(x = as.Date("2020-07-09"), label = "Mask Mandate", vjust = -0.5,
                 y = 30), colour = "black", angle = 90, text = element_text(size=14)) +
     ...
     geom_text(aes(x = as.Date("2020-08-19"), label = "UM semester start", vjust = -0.5,
                 y = 30), colour = "black", angle = 90, text = element_text(size=14)) +
     geom_label(aes(x = Sys.Date()-7, y = 40, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
...
#ggsave("ep_curve_onset_recent.jpg", width = 11, height = 5)
```
...
#ggsave("C:/R/covid19/missoula_r_plot.png", width = 11, height = 6)
```
...
#ggsave("epi_curve_onset.jpg", width = 11, height = 5)
```
...
```{r, include=FALSE, eval=FALSE}
## Missoula county R results using state data
## Using State data to be able to incorporate local vs imported cases
## Only updates once per week

case_data <- read_xlsx(paste0(file_path, "Input/SI_Local_v_Import Data_10.07.2020.xlsx"),
                                 sheet = 2) %>% 
   rename_all(tolower) %>% 
   select(-case_no) %>% 
