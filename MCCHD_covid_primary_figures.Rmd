...
date: "Data as of October 5, 2020"
output: pdf_document
header-includes:
...
age_count_plot <- covid_data %>% 
   ...
   ggplot() +   
     ...
     labs(title = "COVID-19 Cases by age group",
          subtitle = paste0("Missoula County, since March 2020. Data incomplete for ", age_na, " cases."), 
...
age_count_month_plot <- covid_data %>% 
   ...
   ggplot() +
      ...
      geom_label(aes(x = Sys.Date()-7, y = 75, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
      theme_minimal() +
      labs(title = "Weekly COVID-19 cases by age group",
        subtitle = paste0("Missoula County, 2020. Data incomplete for ", age_na, " cases."),
        color = "Age group") +
...
```{r}
age_n <- covid_data %>% 
   filter(is.na(age_group3) | is.na(test_date_new))
age_na <- sum(age_n$case)
...
#ggsave("C:/R/covid19/age_count_month_plot2.png", width = 10, height = 6)
```
...
age_incidence_plot_line <- covid_data %>% 
   ...
   ggplot() +
      ...
      xlab("") +
      geom_label(aes(x = Sys.Date()-7, y = 400, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
      scale_x_date(breaks = seq.Date(from = as.Date("2020-08-02"), 
                                    to = as.Date(Sys.Date()), 
                  ...
                  labels = seq.Date(from = as.Date("2020-08-02"), 
                                    ...
                                    by = "7 days"),
                  date_labels = "%d-%b") +
      scale_y_continuous(breaks = c(seq(0, 500, 50)), labels = c(seq(0, 500, 50))) +
      theme(strip.text = element_text(size = 16, colour = "black"),
         title = element_text(size = 18, colour = "black"),
         panel.grid = element_blank(),
         panel.grid.major.y = element_line(colour = "grey"),
...
```{r}
#Bar plot of race incidence data
race_incidence_plot <- race_incidence %>% 
   filter(region == "Missoula County") %>% 
   ggplot() +
     geom_col(aes(fct_rev(race), group_inc), fill = "steelblue", width = .6) +
     coord_flip() +
     labs(title = "COVID-19 incidence by self-reported race",
          subtitle = paste0("Missoula County. Data incomplete for ", race_na, " cases."),
          y = "Incidence per 100,000 Population", x = "") +
     scale_y_continuous(breaks = c(seq(0, 10000, 500)), labels = c(seq(0, 10000, 500))) +
     theme_minimal() +
     theme(strip.text = element_text(size = 16, colour = "black"),
           title = element_text(size = 18, colour = "black"),
           panel.grid = element_blank(),
...
ethnicity_n <- covid_data %>% 
   filter(is.na(ethnicity))
ethnicity_na <- sum(ethnicity_n$case)
...
ethnicity_plot <- covid_data %>% 
   ...
   ggplot() +   
     ...
     labs(title = "COVID-19 cases by self-reported ethnicity",
          subtitle = paste0("Missoula County. Data incomplete for ", ethnicity_na, " cases."),
          y = "Count", x = "", fill = "") +
     scale_y_continuous(breaks = c(seq(0, 1000, 50)), labels = c(seq(0, 1000, 50))) +
     coord_flip() +
     theme_minimal() +
...
ethnicity_incidence_plot <- ethnicity_incidence %>% 
   ...
   ggplot() +
     ...
     labs(title = "COVID-19 incidence by self-reported ethnicity",
          subtitle = paste0("Missoula County. Data incomplete for ", ethnicity_na, " cases."),
          y = "Incidence per 100,000 Population", x = "") +
     scale_y_continuous(breaks = c(seq(0, 1200, 50)), labels = c(seq(0, 1200, 50))) +
     theme_minimal() +
     theme(strip.text = element_text(size = 16, colour = "black"),
           title = element_text(size = 18, colour = "black"),
           panel.grid = element_blank(),
...
symptoms_gender_plot <- covid_data %>% 
   ...
   ggplot(aes(x = fct_rev(variable), y = values, fill = gender)) +   
     ...
     labs(title = "Symptoms reported at time of testing", 
          ...
          y = "Count", x = "", fill = "") +
     scale_y_continuous(breaks = c(seq(0, 600, 25)), labels = c(seq(0, 600, 25))) +
     coord_flip() +
     theme_minimal() +
     theme(strip.text = element_text(size = 16, colour = "black"),
           title = element_text(size = 18, colour = "black"),
...
symptoms_age_plot <- covid_data %>% 
   ...
   ggplot(aes(x = fct_rev(variable), y = values, fill = age_group)) +   
     geom_bar(stat = "identity", width = .6) +   
     labs(title = "Symptoms reported at time of testing", 
          subtitle = "By age group, Missoula County, 2020",
          y = "Count", x = "", fill = "") +
     scale_y_continuous(breaks = c(seq(0, 600, 25)), labels = c(seq(0, 600, 25))) +
     coord_flip() +
     theme_minimal() +
...
# Line plot of number of contacts per age group by month
contacts_age_plot <- covid_data %>% 
   filter(test_date_new > "2020-08-01") %>% 
   mutate(month_of_year = month(test_date_new, label = TRUE)) %>% 
   mutate(week_of_year = cut.Date(test_date_new, breaks = "1 week", start.on.monday = FALSE),
          week_of_year = ymd(week_of_year)) %>% 
   ...
   ggplot() +
      ...
      geom_label(aes(x = Sys.Date()-7, y = 15, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
      geom_line(aes(week_of_year, mean_contacts, group = age_group, 
                    color = age_group), size = 1.5) +
      theme_minimal() +
      labs(title = "Mean number of contacts per COVID case, by week",
        subtitle = paste0("Missoula County, 2020. Data incomplete for ", age_na, " cases."),
        color = "Age group") +
      ylab("Mean # contacts") +
      xlab("") +
      scale_x_date(breaks = seq.Date(from = as.Date("2020-08-02"), 
                  ...
                  labels = seq.Date(from = as.Date("2020-08-02"), 
                                    ...
                                    by = "7 days"),
                  date_labels = "%d-%b") +
      scale_y_continuous(breaks = c(seq(0, 25, 5)), labels = c(seq(0, 25, 5))) +
      theme(strip.text = element_text(size = 16, colour = "black"),
         title = element_text(size = 18, colour = "black"),
         panel.grid = element_blank(),
         panel.grid.major.y = element_line(colour = "grey"),
...
#ggsave("C:/R/covid19/contacts_age_plot.png", width = 10, height = 6)
```
...
epi_curve_onset_recent <- plot_data %>% 
   ...
   ggplot() +
     ...
     geom_text(aes(x = as.Date("2020-07-09"), label = "Mask Mandate", vjust = -0.5,
                 y = 20), colour = "black", angle = 90, text = element_text(size=14)) +
     ...
     geom_label(aes(x = Sys.Date()-7, y = 20, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
     labs(title = "COVID-19 Cases in Missoula County Since July 1, 2020",
...
plot_data <- covid_data %>% 
   filter(test_date_new > "2020-06-30") 
...
epi_curve_test_date <- plot_data %>% 
   ggplot() +
     ...
     geom_vline(xintercept = as.numeric(as.Date("2020-07-09")), color = "black", 
              size = 1, linetype = "dashed") +
     geom_text(aes(x = as.Date("2020-07-09"), label = "Mask Mandate", vjust = -0.5,
                 y = 25), colour = "black", angle = 90, text = element_text(size=14)) +
     geom_vline(xintercept = as.numeric(as.Date("2020-08-19")), color = "black", 
              size = 1, linetype = "dashed") +
     geom_text(aes(x = as.Date("2020-08-19"), label = "UM semester start", vjust = -0.5,
                 y = 25), colour = "black", angle = 90, text = element_text(size=14)) +
     geom_label(aes(x = Sys.Date()-12, y = 20, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
     labs(title = "COVID-19 Cases in Missoula County Since July 1, 2020",
          subtitle = paste0("Cases in plot = ", total_cases),
...
case_data <- read_xlsx(paste0(file_path, "Input/SI_Local_v_Import Data_10.01.2020.xlsx"),
                                 sheet = 2) %>% 
   rename_all(tolower) %>% 
   select(-case_no) %>% 
...
latest_date <- format(Sys.Date() - 19, "%Y-%m-%d")
...
epi_curve_onset <- covid_data %>% 
   ...
   ggplot() +
     ...
     geom_text(aes(x = as.Date("2020-03-16"), label = "Schools close", vjust = -0.5,
                 y = 15), colour = "black", angle = 90, text = element_text(size=14)) +
...
epi_curve_test_date <- covid_data %>% 
   ggplot() +
     geom_rect(aes(xmin = Sys.Date() - 14, xmax = Sys.Date(), ymin=0, ymax=Inf),
               fill = "gray80", alpha = 0.05) +
     geom_col(aes(test_date_new, case, fill = test_date_indicator)) +
...
############### Older plots included here for future reference #########################
