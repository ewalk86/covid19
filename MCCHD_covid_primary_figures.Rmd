...
date: "Data as of October 26, 2020"
output: pdf_document
header-includes:
...
race_plot_count <- covid_data %>% 
   ...
   ggplot() +   
     ...
     labs(title = "COVID-19 cases by self-reported race",
          subtitle = paste0("Missoula County. N = ", race_cases, " cases."),
          y = "Count", x = "", fill = "") +
...
#ggsave("symptoms_gender_plot.jpg", width = 12, height = 6)
```
...
#ggsave("piechart_contacts.jpg", width = 10, height = 10)
```
...
#ggsave("C:/R/covid19/contacts_age_plot.png", width = 12, height = 6)
```
...
epi_curve_onset <- plot_data %>% 
   ...
   ggplot() +
     ...
     geom_text(aes(x = as.Date("2020-08-19"), label = "UM semester start", vjust = -0.5,
                 y = 30), colour = "black", angle = 90, text = element_text(size=14)) +
     geom_label(aes(x = Sys.Date()-10, y = 30, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
...
```{r}
# Line plot of days between symptom onset and test results
date_n <- covid_data %>% 
   filter(test_date_new > "2020-08-01") %>% 
   filter(!is.na(symptom_test_diff) & !is.na(test_date_new))
date_cases <- sum(date_n$case)


symptom_test_date_plot <- covid_data %>% 
   filter(test_date_new > "2020-08-01") %>% 
   #mutate(month_of_year = month(test_date_new, label = TRUE)) %>% 
   mutate(week_of_year = cut.Date(test_date_new, breaks = "1 week", start.on.monday = FALSE),
          week_of_year = ymd(week_of_year)) %>% 
   group_by(week_of_year) %>% 
   mutate(mean_test_diff = mean(symptom_test_diff, na.rm = TRUE)) %>% 
   filter(!is.na(mean_test_diff)) %>% 
   filter(!is.na(week_of_year)) %>% 
   distinct(mean_test_diff, week_of_year) %>% 
   ggplot() +
      geom_rect(aes(xmin = Sys.Date() - 14, xmax = Sys.Date(), ymin=0, ymax=Inf),
               fill = "gray80", alpha = 0.05) +
      geom_label(aes(x = Sys.Date()-8, y = 3, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
      geom_line(aes(week_of_year, mean_test_diff), size = 1.5) +
      theme_minimal() +
      labs(title = "Mean days from symptom onset to test results, by week",
        subtitle = paste0("Missoula County, 2020. N = ", date_cases, " cases."),
        color = "Age group") +
      ylab("Mean # contacts") +
      xlab("") +
      scale_x_date(breaks = seq.Date(from = as.Date("2020-08-02"), 
...
symptom_test_date_plot

#ggsave("C:/R/covid19/symptom_test_date_plot.png", width = 12, height = 6)
```
