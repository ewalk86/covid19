...
covid_data <- read_rds(paste0(output_path, output_file))
```
...
age_group_summary <- covid_data %>% 
   ...
   mutate("Proportion of Cases (%)" = 
          round(`Number of Individuals`/(sum(`Number of Individuals`))*100),
          "Rate per 10,000 Population" = 
          round(`Number of Individuals`*age_group_pop/10000)) %>% 
...
gender_summary <- covid_data %>% 
   ...
   mutate("Proportion of Cases (%)" = 
          round(`Number of Individuals`/(sum(`Number of Individuals`))*100),
          "Rate per 10,000 Population" = 
          round(`Number of Individuals`*gender_pop/10000)) %>% 
...
covid_summary <- rbind(age_group_summary, gender_summary,
...
kable(covid_summary, 
   ...
   kable_styling() %>% 
   pack_rows("Age Group (Years)", 1, 2) %>% 
   pack_rows("Gender", 3, 4) %>% 
   pack_rows("Travel History", 5, 6) %>% 
...
covid_plot_onset <- covid_data %>% 
   ggplot() +
     geom_col(aes(symptom_onset_date_new, case, fill = symptom_onset_date_indicator), 
              color = "black") +
     labs(title = "Number of infections by date of symptom onset", 
          fill = "Onset date method") +
   ...
   geom_vline(xintercept = as.numeric(as.Date("2020-03-16")), color = "red", 
              size = 1.2) +
   geom_text(aes(x = as.Date("2020-03-16"), label = "Schools close", vjust = -0.5,
                 y = 3.5), colour = "black", angle = 90, text = element_text(size=14)) +
   geom_vline(xintercept = as.numeric(as.Date("2020-03-28")), color = "red", 
              size = 1.2) +
   geom_text(aes(x = as.Date("2020-03-28"), label = "Stay at home order", vjust = -0.5,
                 y = 3.5), colour = "black", angle = 90, text = element_text(size=14)) 
covid_plot_onset %>% 
...
covid_plot_age <- covid_data %>% 
   ggplot() +
     geom_col(aes(symptom_onset_date_new, case, fill = age_group), color = "black") +
     labs(title = "Number of infections by date of symptom onset", 
          fill = "Age group") +
     scale_x_date(date_breaks = "2 days") +
     theme_minimal() +
...
covid_plot_gender <- covid_data %>% 
   ggplot() +
     geom_col(aes(symptom_onset_date_new, case, fill = gender), color = "black") +
     labs(title = "Number of infections by date of symptom onset", fill = "Gender") +
     scale_x_date(date_breaks = "2 days") +
     theme_minimal() +
     theme(strip.text = element_text(size = 16, colour = "black"),
           title = element_text(size = 18, colour = "black"),
...
covid_data_table <- covid_data %>% 
   mutate(ventilated = as.character(ventilated),
          ventilated = if_else(is.na(ventilated), "Not hospitalized", ventilated))

save_table <- data.frame(table(covid_data_table$hospitalized, 
                               covid_data_table$ventilated)) %>% 
   filter(Freq > 0)
