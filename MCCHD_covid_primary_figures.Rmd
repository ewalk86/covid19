...
date: "Data as of September 17, 2020"
output: pdf_document
header-includes:
...
covid_plot <- covid_data %>% 
   ...
   ggplot(aes(x = fct_rev(variable), y = values, fill = gender)) +   
     ...
     labs(title = "Symptoms reported at time of testing", 
          subtitle = "By gender",
          y = "Count", x = "", fill = "") +
     scale_y_continuous(breaks = c(seq(0, 300, 10)), labels = c(seq(0, 300, 10))) +
     coord_flip() +
     theme_minimal() +
     theme(strip.text = element_text(size = 16, colour = "black"),
           title = element_text(size = 18, colour = "black"),
...
covid_plot_onset <- covid_data %>% 
   ...
   ggplot() +
     ...
     geom_label(aes(x = Sys.Date()-7, y = 8, 
...
case_data <- read_xlsx(paste0(file_path, "Input/SI_Local_v_Import Data_09.17.2020.xlsx"),
                                 sheet = 2) %>% 
   rename_all(tolower) %>% 
   select(-case_no) %>% 
   ...
   filter(dates > "2020-05-30" & dates < Sys.Date() - 14) %>% 
   arrange(dates) %>% 
   ungroup() %>% 
...
latest_date <- format(Sys.Date() - 15, "%Y-%m-%d")
