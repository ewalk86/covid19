...
date: "Data as of August 16, 2021"
output: pdf_document
header-includes:
...
age_n <- covid_data %>% 
   ...
   filter(test_date > "2020-12-31")
age_cases <- sum(age_n$case)

# Age group line plot - counts by month
age_count_month_plot <- covid_data %>% 
   filter(test_date > "2020-12-31" & test_date < Sys.Date()) %>% 
   ...
   ggplot() +
      ...
      geom_label(aes(x = Sys.Date()-21, y = 80, 
      ...
      xlab("") +
      scale_x_date(breaks = seq.Date(from = as.Date("2021-01-01"), 
                                    to = as.Date(Sys.Date()), 
                                    by = "7 days"),
                  labels = seq.Date(from = as.Date("2021-01-01"), 
                                    to = as.Date(Sys.Date()), 
                                    by = "7 days"),
                  date_labels = "%d-%b") +
...
age_count_month_plot2 <- covid_data %>% 
   ...
   ggplot() +
      ...
      geom_label(aes(x = Sys.Date()-21, y = 50, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
      theme_minimal() +
      labs(title = "Weekly COVID-19 cases by age group",
...
age_incidence_plot_line <- covid_data %>% 
   ...
   ggplot() +
      ...
      xlab("") +
      geom_label(aes(x = Sys.Date()-21, y = 400, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
      scale_x_date(breaks = seq.Date(from = as.Date("2021-01-01"), 
                                    to = as.Date(Sys.Date()), 
...
epi_curve_test_date <- plot_data %>% 
   ggplot() +
     ...
     geom_label(aes(x = Sys.Date()-18, y = 70, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
...
epi_curve_full <- plot_data %>% 
   ...
   ggplot() +
     ...
     geom_text(aes(x = as.Date("2020-12-28"), label = "Christmas + New Years", vjust = -0.5,
                 y = 80), colour = "black", angle = 90, text = element_text(size=14)) +
     geom_label(aes(x = Sys.Date()-25, y = 120, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
