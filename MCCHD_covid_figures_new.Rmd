...
date: "Data as of November 23, 2020"
output: pdf_document
header-includes:
...
case_data <- covid_data %>% 
   ...
   filter(!is.na(dates)) %>% 
   filter(dates > "2020-06-30" & dates < Sys.Date() - 6) %>% 
   arrange(dates) %>% 
   select(dates, case) %>% 
   arrange(dates) %>% 
   ungroup() 

latest_date <- format(Sys.Date() - 7, "%Y-%m-%d")
