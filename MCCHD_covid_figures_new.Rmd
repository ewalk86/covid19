...
age_n <- covid_data %>% 
   ...
   filter(test_date > "2021-12-31")
age_cases <- sum(age_n$case)

# Age group line plot - counts by month
age_count_month_plot <- covid_data %>% 
   filter(test_date > "2021-12-31" & test_date < Sys.Date()) %>% 
   ...
   ggplot() +
      ...
      labs(title = "Weekly COVID-19 cases by age group",
        subtitle = paste0("Missoula County, 2022. N = ", age_cases, " cases."),
        color = "Age group") +
      ylab("Number of cases per week") +
      xlab("") +
      scale_x_date(breaks = seq.Date(from = as.Date("2022-01-01"), 
                                    to = as.Date(Sys.Date()), 
                                    by = "7 days"),
                  labels = seq.Date(from = as.Date("2022-01-01"), 
                                    to = as.Date(Sys.Date()), 
                                    by = "7 days"),
                  date_labels = "%d-%b") +
...
age_incidence_plot_bar <- age_incidence %>% 
   ...
   ggplot() +
     ...
     scale_y_continuous(breaks = c(seq(0, 50000, 2000)), labels = c(seq(0, 50000, 2000))) +
     theme_minimal() +
     theme(strip.text = element_text(size = 16, colour = "black"),
           title = element_text(size = 18, colour = "black"),
           panel.grid = element_blank(),
...
epi_curve_test_date <- plot_data %>% 
   ggplot() +
     ...
     geom_label(aes(x = Sys.Date()-21, y = 350, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
     labs(title = "COVID-19 Cases in Missoula County Since January 1, 2022",
...
case_data <- covid_data %>% 
   rename(dates = test_date) %>% 
   filter(!is.na(dates)) %>% 
   filter(dates > "2021-12-31" & dates < Sys.Date() - 13) %>% 
   arrange(dates) %>% 
   select(dates, case) %>% 
   arrange(dates) %>% 
   ungroup() 
...
epi_curve_full <- plot_data %>% 
   ...
   ggplot() +
     ...
     geom_text(aes(x = as.Date("2020-05-28"), label = "Memorial Day + Phase 2 reopening", vjust = -0.5,
                 y = 220), colour = "black", angle = 90, text = element_text(size=14)) +
     ...
     geom_text(aes(x = as.Date("2020-07-06"), label = "July 4th + Mask Mandate", vjust = -0.5,
                 y = 220), colour = "black", angle = 90, text = element_text(size=14)) +
     geom_vline(xintercept = as.numeric(as.Date("2020-09-01")), color = "black", 
              size = 1, linetype = "dashed") +
     geom_text(aes(x = as.Date("2020-09-01"), label = "Fall Semester + Labor Day", vjust = -0.5,
                 y = 250), colour = "black", angle = 90, text = element_text(size=14)) +
...
incidence_plot_line <- covid_data %>% 
   ...
   ggplot() +
      ...
      ylab("Incidence per 100,000") +
      xlab("") +
      geom_label(aes(x = Sys.Date()-50, y = 1000, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
      scale_x_date(breaks = seq.Date(from = as.Date("2020-03-06"), 
                                    to = as.Date(Sys.Date()), 
...
incidence_plot_line <- covid_data %>% 
   ...
   ggplot() +
      ...
      ylab("Incidence per 100,000") +
      xlab("") +
      geom_label(aes(x = Sys.Date()-50, y = 25, 
                    label = "Cases from the shaded \n dates may not yet be \n reflected in the graph."),
                fill = NA) +
      scale_x_date(breaks = seq.Date(from = as.Date("2020-03-06"), 
                                    to = as.Date(Sys.Date()), 
