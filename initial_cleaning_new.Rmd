...
author: "Ethan Walker"
date: "Started 23 Nov 2020, Updated 16 August 2021"
output:
...
covid_data_initial <- 
   read_xlsx(paste0(input_path, "COVID Data 8.16.2021.xlsx"), sheet = 1,
...
variant_data_initial <- 
   read_xlsx(paste0(input_path, "Breakthroughs Reinfections and Variants 8.12.2021.xlsx"), sheet = 1, 
             na = c(" ", "", "n/a")) 
...
variant_data_clean <- variant_data_initial %>% 
   ...
   mutate(breakthrough = fct_collapse(`Vaccine Breakthrough?`,
          ...
          case_episode = as.factor(case_episode),
          case_id = as.factor(case_id)) %>% 
   select(row_id, age, followup_status, case_episode:variant_classification) %>% 
   arrange(row_id) %>% 
