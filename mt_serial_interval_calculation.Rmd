...
mt_si_data_pre_sip <- mt_si_data %>% 
   filter(infector_date < "2020-03-27") %>% 
   #filter(infected_date < "2020-04-10") %>% 
   dplyr::select(EL, ER, SL, SR) %>% 
   mutate(SR = if_else(SR == 0, 1, SR)) %>% 
   filter(SR < 29) %>% 
   #mutate(type = if_else(SL == SR, 2, 1)) %>% 
   mutate_all(as.integer) 
...
mt_si_data_sip <- mt_si_data %>% 
   filter(infector_date > "2020-03-27" & infector_date < "2020-04-24") %>% 
...
mt_si_data_phase1 <- mt_si_data %>% 
   filter(infector_date > "2020-04-25" & infector_date < "2020-05-31") %>% 
...
mt_si_data_phase2 <- mt_si_data %>% 
   filter(infector_date > "2020-05-31" & infector_date < "2020-07-31") %>% 
...
mt_r_results <- estimate_R(mt_li_inc_data, method = "si_from_data", 
                           si_data = mt_si_data_phase2, config = si_config)
```
...
# 95% CI for means
qgamma(c(0.025, 0.975), shape = 405.7, scale = 0.0132)

# Shape and scale of distribution of SDs
egamma(total_results$SI.Moments$Std, ci = TRUE)
# 95% CI for SDs
qgamma(c(0.025, 0.975), shape = 260.7, scale = 0.0175)
