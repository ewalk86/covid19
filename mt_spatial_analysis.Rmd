...
county_data <- cleaning_data %>% 
   ...
   select(-mt_case, -case, -case_no, -dates) %>% 
   ...
   select(state_cases, state_inc, county, region, county_cases, county_inc, county_state_inc_ratio,
...
inc_ratio_summary <- county_data %>% 
   select(county, county_state_inc_ratio) %>% 
