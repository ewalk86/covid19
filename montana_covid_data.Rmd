---
title: "State of Montana Covid-19 Data"
author: "Ethan Walker"
date: "Data as of 13 May 2020"
output:
...
priver <- county_function("Powder River")
...
hosp_data_daily <- state_case_data_clean %>% 
   ...
   mutate(total_hosp = sum(case)) %>% 
   group_by(dates) %>% 
   select(-case) %>% 
   distinct(dates, hospitalization, .keep_all = TRUE) %>% 
   pivot_wider(names_from = "hospitalization", values_from = "total_hosp")
...
```{r}
# Look at mock data for example of formatting
data("MockRotavirus")
...
mt_si_data <- read_csv(paste0(file_path, "Input/si_data.csv")) %>% 
   select(-Pair_No) %>% 
...
mt_li_data <- read_csv(paste0(file_path, "Input/local_import_data.csv")) %>% 
   ...
   mutate(local = if_else(Local_Import == 0, 1, 0),
          ...
          dates = mdy(`Date Reported to CDEpi`)) %>% 
   group_by(dates) %>% 
...
## State incidence data
date_today <- Sys.Date()
state_incidence <- incidence(state_data_clean$dates, last_date = date_today)

# Merge local/import data with state incidence data to get all dates in the range
mt_li_data_final <- as.data.frame(state_incidence$dates) %>% 
   mutate(dates = ymd(state_incidence$dates)) %>% 
   select(dates) %>% 
...
r_mt_si <- estimate_R(mt_li_data_final, method = "si_from_data", 
    si_data = mt_si_data, config = si_config)

estimate_plots <- estimate_R_plots(r_mt_si, what = "R")
estimate_plots

mt_mean_si <- r_mt_si$SI.Moments %>% 
...
colomns <- c(1:4, 8)
state_r <- (r_mt_si$R[,colomns]) %>% 
   mutate(region = "state") %>% 
   rename(mean_r = `Mean(R)`,
          sd_r = `Std(R)`,
          median_r = `Median(R)`)

state_dates <- as.data.frame(r_mt_si$dates)
state_i <- as.data.frame(r_mt_si$I)
state_cil <- as.data.frame(r_mt_si$R$`Quantile.0.025(R)`)
state_cih <- as.data.frame(r_mt_si$R$`Quantile.0.975(R)`)
state_dates_new <- cbind(state_dates, state_i) %>% 
   rename(dates = 1,
          incidence = 2) %>% 
   mutate(dates = ymd(dates))
