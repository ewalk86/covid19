...
state_data_clean <- state_data %>% 
   ...
   mutate(age_group2 = if_else(age_group == "80-89" | age_group == "90-99",
          ...
          age_group2 = factor(age_group2, 
                              labels = c("0-9, 0.12", "10-19, 0.13", "20-29, 0.13", 
                                         "30-39, 0.13", "40-49, 0.11", "50-59, 0.13", 
                                         "60-69, 0.14", "70-79, 0.08", "80+, 0.04"))) %>% 
   separate(age_group2, c("age_group2", "age_group2_percent"), sep = ", ") %>% 
   mutate(age_group2_percent = as.numeric(age_group2_percent),
...
county_data_new <<- county_dates %>% 
   ...
   select(-"NA") %>% 
   group_by(dates) %>% 
   pivot_wider(names_from = "outcome", values_from = "case2") %>% 
   select(-"NA") %>% 
   group_by(dates) %>% 
   ...
   select(-case_no, -age_group, -sex, -mt_case, -age_group2,
          -age_group2_percent, -state_pop)
