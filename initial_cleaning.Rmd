...
author: "Ethan Walker"
date: "Started 2 April 2020, Updated 23 June 2020"
output:
...
covid_data_initial <- 
   read_xlsx(paste0(input_path, "COVID data 6.30.20.xlsx"), 
             range = "A2:X500", 
                        col_names = c("case_id", "age", "gender", "zipcode", "healthcare_worker",
                                      "symptoms", "symptom_type", "symptom_onset_date",
                                      "test_date", "last_exposure_date", 
...
covid_clean <- covid_data_initial %>% 
   ...
   mutate(case = 1,
          ...
          community_spread = if_else(travel_hx == "No" & contact_with_case == "No",
                                     "Yes", "No"),
          community_spread = factor(community_spread, levels = c("Yes", "No")),
          patient_died = factor(patient_died, levels = c("y", "n"),
                             labels = c("Yes", "No")),
          epi_linked = factor(epi_linked, levels = c("y", "n"),
                             labels = c("Yes", "No"))) %>% 
   ...
   mutate(symptom_test_diff = as.duration(interval(symptom_onset_date, test_date)),
          ...
          test_date_indicator = if_else(is.na(test_date),
                                                 "Epi-linked: date of symptom onset",
