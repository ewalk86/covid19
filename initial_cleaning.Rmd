...
author: "Ethan Walker"
date: "Started 2 April 2020, Updated 13 April 2020"
output:
...
```{r}
input_path <- c("C:/Users/ethan.walker/Box/Ethan Walker UM/R/covid19/Input/")
input_file <- c("mcchd_covid_data_20200405.xlsx")


covid_data_initial <- read_xlsx(paste0(input_path, input_file), skip = 1,
                        col_names = c("dob", "gender", "zipcode", "healthcare_worker",
                                      "symptoms", "symptom_type", "symptom_onset_date",
                                      "symptom_onset_comment", "test_date", "test_lab",
...
covid_data_initial <- read_xlsx("Input/mcchd_covid_data_20200406.xlsx", skip = 1,
                        col_names = c("dob", "gender", "zipcode", "healthcare_worker",
                                      "symptoms", "symptom_type", "symptom_onset_date",
                                      "symptom_onset_comment", "test_lab",
                                      "release_date", "contact_with_case", "travel_hx",
                                      "hospitalized", "ventilated", "discharge_date",
                                      "chronic_conditions", "chronic_conditions_type",
                                      "number_contacts", "patient_died", 
...
covid_clean <- covid_data_initial %>% 
   ...
   mutate(case_number = as.numeric(rowname),
          ...
          age_group = cut(age, breaks = c(0, 18, 30, 40, 50, 65, 110),
                          labels = c("<18", "18-29", "30-39", "40-49",
                                     "50-64", "65+"), right = FALSE),
          age_group2 = cut(age, breaks = c(0, 18, 65, 110),
                          labels = c("<18", "18-64", "65+"), right = FALSE)) %>% 
   select(case_number, dob, age, age_group, age_group2, gender:contact_case_number) %>% 
   ...
   mutate(symptom_cough = grepl("COUGH|MILD COUGH", symptom_type, ignore.case = TRUE),
          ...
          symptom_nv = grepl("NAUSEA|N/V", symptom_type, ignore.case = TRUE),
          ...
          symptom_diarrhea = grepl("DIARRHEA", symptom_type, ignore.case = TRUE),
          ...
          symptom_body_aches = grepl("BODY ACHES", symptom_type, ignore.case = TRUE),
          symptom_headache = grepl("HEADACHE|HA", symptom_type, ignore.case = TRUE),
          ...
          symptom_fever_chill = grepl("FEVER|CHILL|NIGHT SWEAT", symptom_type, ignore.case = TRUE),
          symptom_fatigue_lethargy = grepl("FATIGUE|LETHARGY", symptom_type, ignore.case = TRUE),
          symptom_nv_diarrhea = grepl("NAUSEA|N/V|DIARRHEA", symptom_type, ignore.case = TRUE),
          symptom_aches = grepl("BODY ACHES|HEADACHE|MYALGIA|HA", symptom_type, ignore.case = TRUE),
          symptom_sinus_throat = grepl("CONGESTION|RUNNY NOSE|SNEEZING|SORE THROAT|RHINORRHEA|SCRATCHY THROAT", 
   ...
   mutate(symptom_test_diff = as.duration(interval(symptom_onset_date, test_date)),
          ...
          symptom_onset_date_new = as.POSIXct(symptom_onset_date),
          symptom_onset_date_new = if_else(is.na(symptom_onset_date_new), 
                                       test_date - mean_symptom_test_diff*86400,
   ...
   separate(symptom_onset_date_new, c("symptom_onset_date_new", "trash"), sep = " ") %>% 
   mutate(symptom_onset_date_new = ymd(symptom_onset_date_new)) %>% 
   select(-trash)
```

```{r}
output_path <- c("C:/Users/ethan.walker/Box/Ethan Walker UM/R/covid19/Output/")
output_file <- c("covid_data_clean.rds")

write_rds(covid_clean, paste0(output_path, output_file))
```
