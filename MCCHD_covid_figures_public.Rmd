...
kable(covid_summary,
   ...
   pack_rows("Reported Travel Outside Missoula County", 6, 8) %>% 
   pack_rows("Reported Contact with Known Case", 9, 11) %>% 
   pack_rows("Community Spread", 12, 13) %>% 
   footnote(c("Community Spread: no history of travel outside county and no known contact with a known case.",
     "Incidence: the population within age or gender groups is different, so comparing",
     "'Number' or 'Proportion of Cases' can be misleading. 'Incidence per 100,000 Population'",
     "is comparing the number of cases you would expect for each age or gender group if the",
     "groups had the same population – in this case, 100,000 people."))
```
...
covid_plot_onset <- covid_data %>% 
   ggplot() +
     ...
     geom_label(aes(x = Sys.Date()-6, y = 4.5, 
