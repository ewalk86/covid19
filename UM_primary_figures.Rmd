...
kable(covid_summary, caption = "University of Montana COVID-19 Cases",
...
age_count_plot <- covid_data %>% 
   ...
   ggplot() +   
     ...
     labs(title = "COVID-19 Cases by age group",
          subtitle = paste0("University of Montana, since Aug 2020. Age under investigation for ", age_na, " cases."), 
...
age_incidence_plot_bar <- age_incidence %>% 
   ...
   ggplot() +
     ...
     labs(title = "COVID-19 incidence by age group",
          subtitle = paste0("University of Montana, since Aug 2020. Age under investigation for ", age_na, " cases."),
          y = "Incidence per 100,000 Missoula County Population", x = "Age Group (years)") +
...
race_plot_count <- covid_data %>% 
   ...
   ggplot() +   
     ...
     labs(title = "COVID-19 cases by self-reported race",
          subtitle = paste0("University of Montana. Race unknown for ", race_na, " cases."),
          y = "Count", x = "", fill = "") +
...
race_incidence_plot <- race_incidence %>% 
   ...
   ggplot() +
     ...
     labs(title = "COVID-19 incidence by self-reported race",
          subtitle = paste0("University of Montana. Race unknown for ", race_na, " cases."),
          y = "Incidence per 100,000 Missoula County Population", x = "") +
     scale_y_continuous(breaks = c(seq(0, 2000, 200)), labels = c(seq(0, 2000, 200))) +
     theme_minimal() +
     theme(strip.text = element_text(size = 16, colour = "black"),
           title = element_text(size = 18, colour = "black"),
           panel.grid = element_blank(),
...
ethnicity_plot <- covid_data %>% 
   ...
   ggplot() +   
     ...
     labs(title = "COVID-19 cases by self-reported ethnicity",
          subtitle = paste0("University of Montana. Ethnicity unknown for ", race_na, " cases."),
          y = "Count", x = "", fill = "") +
...
ethnicity_incidence_plot <- ethnicity_incidence %>% 
   ...
   ggplot() +
     ...
     labs(title = "COVID-19 incidence by self-reported ethnicity",
          subtitle = paste0("University of Montana. Ethnicity unknown for ", race_na, " cases."),
          y = "Incidence per 100,000 Missoula County Population", x = "") +
     scale_y_continuous(breaks = c(seq(0, 400, 25)), labels = c(seq(0, 400, 25))) +
     theme_minimal() +
     theme(strip.text = element_text(size = 16, colour = "black"),
           title = element_text(size = 18, colour = "black"),
           panel.grid = element_blank(),
...
covid_piechart <- chart_data %>% 
   ggplot(aes("", group_n, fill = chart_var)) +
     ...
     geom_text(aes(label = group_lab), 
               ...
               color = "white", size = 6) +
     labs(title = "University of Montana COVID-19 cases", fill = "",
          subtitle = var_label) + 
     theme_void() +
...
contacts_age_plot <- covid_data %>% 
   ...
   ggplot() +
      geom_line(aes(month_of_year, mean_contacts, group = age_group, 
                    color = age_group), size = 1.5) +
   theme_minimal() +
   labs(title = "Mean number of contacts per COVID case, University of Montana",
        subtitle = "By age group, per month, since Aug 2020",
        color = "Age group") +
   ylab("Mean # contacts") +
   xlab("") +
   theme(strip.text = element_text(size = 16, colour = "black"),
...
epi_curve_onset_recent <- covid_data %>% 
   ...
   ggplot() +
     ...
     geom_text(aes(x = as.Date("2020-08-19"), label = "Semester start", vjust = -0.5,
...
um_data <- covid_data %>% 
   select(symptom_onset_date_new) %>% 
   filter(!is.na(symptom_onset_date_new)) %>% 
   filter(symptom_onset_date_new < Sys.Date() -14)

um_incidence <- incidence(um_data$symptom_onset_date_new)


# Set dates for 14-day rolling averages
um_n <- as.data.frame(um_incidence$dates)
time_var <- nrow(um_n)
...
r_results <- estimate_R(um_incidence, method="parametric_si", 
...
colomns <- c(1:4, 8)
um_r <- (r_results$R[,colomns]) %>% 
   mutate(region = "University of Montana") %>% 
   rename(mean_r = `Mean(R)`,
          sd_r = `Std(R)`,
          median_r = `Median(R)`)

um_dates <- as.data.frame(r_results$dates)
um_i <- as.data.frame(r_results$I)
um_cil <- as.data.frame(r_results$R$`Quantile.0.025(R)`)
um_cih <- as.data.frame(r_results$R$`Quantile.0.975(R)`)
um_dates_new <- cbind(um_dates, um_i) %>% 
   rename(dates = 1,
          incidence = 2) %>% 
   mutate(dates = ymd(dates))
um_dates_new <- um_dates_new[-(1:14), 1:2]
um_dates_new <- cbind(um_dates_new, um_cil, um_cih) %>% 
   rename(cl_low = 3,
          cl_high = 4)
um_r <- cbind(um_r, um_dates_new)
...
um_r_plot <- um_r %>% 
   ...
   labs(title = "COVID-19 Rolling 14-day R-number, University of Montana",
        color = "") +
   ylab("R-number") +
   xlab("") +
